...
<div id="editor_bottom_toolbar" style="display:none">
    ...
    <select name="user_id" class="form-control" data-parsley-exclude="true">
        ...
        <option value="{{ Auth::user()->id }}" @if ( ($mailbox->ticket_assignee == App\Mailbox::TICKET_ASSIGNEE_REPLYING_UNASSIGNED && !$conversation->user_id) || ($mailbox->ticket_assignee == App\Mailbox::TICKET_ASSIGNEE_REPLYING) ) data-default="true" selected="selected" @endif>{{ __('Me') }}</option>

        @foreach ($mailbox->usersHavingAccess() as $user)
            @if ($user->id != Auth::user()->id)
            	<option value="{{ $user->id }}" @if ($conversation->user_id == $user->id && $mailbox->ticket_assignee != App\Mailbox::TICKET_ASSIGNEE_REPLYING)data-default="true" selected="selected"@endif>{{ $user->getFullName() }}</option>
            @endif
        @endforeach
    </select> 
